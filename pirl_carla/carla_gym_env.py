...
from pirl_carla.lane_assist import LaneAssist
...
class CarlaEnv:
    ...
    def __init__(self, town='Town10HD', port=2000, render=True):
        try:
            ...
            self.lane_assist = LaneAssist()  # Initialize lane assist
    ...
    def _setup_physics_parameters(self):
        ...
        self.max_speed = 6.0  # maximum speed (m/s)
        self.max_throttle = 1.0
        self.max_brake = 1.0
        self.max_steer = 1.0
        self.min_speed = 1.0  # minimum desired speed (m/s)
        self.stuck_speed_threshold = 0.1  # speed below which vehicle is considered stuck
        self.initial_grace_period = 9000  # steps to allow vehicle to get moving
    ...
    def reset(self) -> np.ndarray:
        """Reset environment and lane assist"""
        self.lane_assist.reset()  # Reset lane assist PID controller
        try:
    ...
    def step(self, action: np.ndarray) -> Tuple[np.ndarray, float, bool, Dict[str, Any]]:
        ...
        try:
            ...
            lane_distance, lane_angle, _ = self.lane_assist.get_lane_info(self.vehicle, self.map)
            speed = state[3]  # Current speed from state
            
            # Calculate lane-keeping steering command
            lane_steering = self.lane_assist.compute_steering(lane_distance, lane_angle, speed)
            
            # Calculate waypoint following steering if available
            ...
            if next_waypoint:
                ...
                waypoint_steering = math.atan2(right_dot, forward_dot)
                waypoint_steering = np.clip(waypoint_steering, -self.max_steer, self.max_steer)
            
            # Blend the two steering commands (70% lane keeping, 30% waypoint following)
            steering = 0.7 * lane_steering + 0.3 * waypoint_steering
    ...
    def _get_observation(self) -> np.ndarray:
        """Get current observation including lane information"""
        ...
        # Get lane information
        lane_distance, lane_angle, road_width = self.lane_assist.get_lane_info(self.vehicle, self.map)
        ...
        return np.array([
            ...
            distance_to_target,
            angle_to_target,
            lane_distance,  # Distance from lane center
            lane_angle,     # Angle relative to lane
            road_width      # Width of the current lane
        ])
    ...
    def _compute_reward(self, state: np.ndarray, action: np.ndarray, 
        ...
        # Check lane invasion - wrapped in try-except to handle edge cases
        try:
            if self.lane_assist.get_lane_invasion_info(self.vehicle, self.map):
                return -100.0  # Penalty for lane invasion
        except Exception as e:
            # If lane invasion check fails, fall back to distance-based penalty
            lane_distance = abs(next_state[6])  # Distance from lane center
            if lane_distance > 2.0:  # If more than 2 meters from lane center
                return -100.0
            
        # Lane keeping reward
        lane_distance = abs(next_state[6])  # Distance from lane center
        lane_angle = abs(next_state[7])     # Angle relative to lane
        lane_keeping_reward = -lane_distance - 2.0 * lane_angle  # Higher penalty for angle deviation
        ...
        # Combine rewards with weights prioritizing lane keeping
        return (
            2.0 * lane_keeping_reward +  # Prioritize lane keeping
            velocity_reward +
            0.5 * distance_reward +
            0.5 * heading_reward +
            steering_penalty +
    ...
    def _check_termination(self) -> bool:
        ...
        # Check if vehicle is off road or in wrong lane
        ...
        try:
            if self.lane_assist.get_lane_invasion_info(self.vehicle, self.map):
                print("Episode terminated due to lane invasion")
                return True
        except Exception as e:
            # If lane invasion check fails, fall back to distance-based check
            lane_distance = abs(self._get_observation()[6])  # Distance from lane center
            if lane_distance > 2.0:  # If more than 2 meters from lane center
                print("Episode terminated due to excessive lane deviation")
                return True
